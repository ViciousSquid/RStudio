...
- Pickup collection
- Monster AI (sight detection, movement, wake logic, shooting, projectiles)
...
class LogicThread(threading.Thread):
    ...
    def __init__(self, game_state: ThreadedGameState,
        ...
        # Active projectiles spawned by flying monsters
        # Each entry: {'pos': glm.vec3, 'vel': glm.vec3, 'damage': int, 'dist': float}
    ...
    # =========================================================================
    # MONSTER COLLISION HELPERS
    # =========================================================================

    def _monster_overlaps_brushes(self, pos: glm.vec3,
                                   hw: float, hh: float, hd: float) -> bool:
        """Return True if a monster AABB centred at *pos* overlaps any solid brush."""
        for brush in self.brushes:
            if (brush.get('hidden') or brush.get('is_water') or
                    brush.get('is_fog') or brush.get('is_trigger')):
                continue
            bp = brush.get('pos',  [0, 0, 0])
            bs = brush.get('size', [64, 64, 64])
            if (pos.x + hw > bp[0] - bs[0] * 0.5 and
                    pos.x - hw < bp[0] + bs[0] * 0.5 and
                    pos.y + hh > bp[1] - bs[1] * 0.5 and
                    pos.y - hh < bp[1] + bs[1] * 0.5 and
                    pos.z + hd > bp[2] - bs[2] * 0.5 and
                    pos.z - hd < bp[2] + bs[2] * 0.5):
                return True
        return False

    def _resolve_monster_y(self, pos: glm.vec3,
                            hw: float, hh: float, hd: float):
        """
        Push monster out of vertical brush overlaps.
        Returns (new_pos: glm.vec3, on_ground: bool).
        """
        on_ground = False
        for brush in self.brushes:
            if (brush.get('hidden') or brush.get('is_water') or
            ...
            bp = brush.get('pos',  [0, 0, 0])
            bs = brush.get('size', [64, 64, 64])
            bmin_x = bp[0] - bs[0] * 0.5;  bmax_x = bp[0] + bs[0] * 0.5
            bmin_y = bp[1] - bs[1] * 0.5;  bmax_y = bp[1] + bs[1] * 0.5
            bmin_z = bp[2] - bs[2] * 0.5;  bmax_z = bp[2] + bs[2] * 0.5

            # Must overlap in XZ first
            if not (pos.x + hw > bmin_x and pos.x - hw < bmax_x and
                    pos.z + hd > bmin_z and pos.z - hd < bmax_z):
                continue

            mmin_y = pos.y - hh
            mmax_y = pos.y + hh
            if mmax_y <= bmin_y or mmin_y >= bmax_y:
                continue

            dy_floor   = bmax_y - mmin_y   # feet below top surface → land on it
            dy_ceiling = mmax_y - bmin_y   # head above bottom surface → hit ceiling

            if dy_floor < dy_ceiling:
                # Stand on top of brush
                pos = glm.vec3(pos.x, bmax_y + hh + 0.001, pos.z)
                on_ground = True
            else:
                # Push down from ceiling
                pos = glm.vec3(pos.x, bmin_y - hh - 0.001, pos.z)

        return pos, on_ground

    def _has_line_of_sight(self, origin: glm.vec3, target: glm.vec3) -> bool:
        """
        Return True if there is an unobstructed line from *origin* to *target*.
        Casts a ray and returns False if any solid brush is hit before the target.
        """
        direction = target - origin
        dist_to_target = glm.length(direction)
        if dist_to_target < 0.001:
            return True
        ray_dir = direction / dist_to_target
        ...
        for brush in self.brushes:
            ...
            bp = brush.get('pos',  [0, 0, 0])
            bs = brush.get('size', [64, 64, 64])
            b_min = glm.vec3(bp[0] - bs[0] * 0.5,
                             bp[1] - bs[1] * 0.5,
                             bp[2] - bs[2] * 0.5)
            b_max = glm.vec3(bp[0] + bs[0] * 0.5,
                             bp[1] + bs[1] * 0.5,
                             bp[2] + bs[2] * 0.5)
            hit, t = self._intersect_ray_aabb(origin, ray_dir, b_min, b_max)
            if hit and t < dist_to_target:
                return False
        return True
    ...
    def _update_monsters(self, delta: float):
        ...
        Once awake:
          - Ground monsters slide toward the player horizontally with brush collision
            and are subject to gravity so they fall off ledges correctly.
          - Flying monsters move in full 3D toward the player with brush collision
            but no gravity — they maintain altitude freely.
          - Ground monsters deal instant damage on their shoot cooldown.
          - Flying monsters spawn a slow-moving projectile instead of instant damage.
        ...
        I/O events fired:
          ...
          - OnAttack     — each time the monster shoots / spawns a projectile
        ...
        for thing in self.things:
            ...
            if mid not in self.monster_states:
                self.monster_states[mid] = {
                    ...
                    'anim_timer':  0.0,
                    'in_sight':    False,
                    'vel_y':       0.0,   # vertical velocity (ground monsters only)
                }
            ...
            # Collision half-extents (same for all types)
            m_hw, m_hh, m_hd = 32.0, 48.0, 32.0
            ...
            if distance <= MONSTER_SIGHT_RANGE:
                ...
                # ---- Movement -----------------------------------------------
                if mtype == 'flying':
                    # Full 3D movement toward player, collision-aware, no gravity
                    if distance > MONSTER_STOP_DISTANCE:
                        to_player = player_pos - thing_pos
                        to_len = glm.length(to_player)
                        if to_len > 0.001:
                            step = (to_player / to_len) * MONSTER_MOVE_SPEED * delta
                            # Axis-separated so monsters slide along surfaces
                            trial = glm.vec3(thing_pos.x + step.x, thing_pos.y, thing_pos.z)
                            if not self._monster_overlaps_brushes(trial, m_hw, m_hh, m_hd):
                                thing_pos = trial
                            trial = glm.vec3(thing_pos.x, thing_pos.y, thing_pos.z + step.z)
                            if not self._monster_overlaps_brushes(trial, m_hw, m_hh, m_hd):
                                thing_pos = trial
                            trial = glm.vec3(thing_pos.x, thing_pos.y + step.y, thing_pos.z)
                            if not self._monster_overlaps_brushes(trial, m_hw, m_hh, m_hd):
                                thing_pos = trial
                    thing.pos = [thing_pos.x, thing_pos.y, thing_pos.z]

                else:
                    # Horizontal movement + gravity; cannot pass through walls or float
                    if distance > MONSTER_STOP_DISTANCE:
                        horiz_dx = player_pos.x - thing_pos.x
                        horiz_dz = player_pos.z - thing_pos.z
                        horiz_len = math.sqrt(horiz_dx * horiz_dx + horiz_dz * horiz_dz)
                        if horiz_len > 0.001:
                            step_x = (horiz_dx / horiz_len) * MONSTER_MOVE_SPEED * delta
                            step_z = (horiz_dz / horiz_len) * MONSTER_MOVE_SPEED * delta
                            trial = glm.vec3(thing_pos.x + step_x, thing_pos.y, thing_pos.z)
                            if not self._monster_overlaps_brushes(trial, m_hw, m_hh, m_hd):
                                thing_pos = trial
                            trial = glm.vec3(thing_pos.x, thing_pos.y, thing_pos.z + step_z)
                            if not self._monster_overlaps_brushes(trial, m_hw, m_hh, m_hd):
                                thing_pos = trial

                    # Apply gravity
                    state['vel_y'] += GRAVITY * delta
                    if state['vel_y'] < TERMINAL_VELOCITY:
                        state['vel_y'] = TERMINAL_VELOCITY
                    thing_pos = glm.vec3(thing_pos.x,
                                        thing_pos.y + state['vel_y'] * delta,
                                        thing_pos.z)

                    # Resolve vertical brush collisions
                    thing_pos, on_ground = self._resolve_monster_y(thing_pos, m_hw, m_hh, m_hd)
                    if on_ground:
                        state['vel_y'] = 0.0
                ...
                # ---- Shooting -----------------------------------------------
                # Aim from the monster's centre toward the player's centre.
                # Only tick the shoot timer and fire when there is a clear
                # line of sight — this prevents shooting through walls.
                monster_eye = glm.vec3(thing_pos.x,
                                       thing_pos.y + m_hh * 0.5,
                                       thing_pos.z)
                player_eye  = glm.vec3(player_pos.x,
                                       player_pos.y,
                                       player_pos.z)
                can_see = self._has_line_of_sight(monster_eye, player_eye)

                if can_see:
                    state['shoot_timer'] -= delta

                if can_see and state['shoot_timer'] <= 0.0:
                    ...
                    if mtype == 'flying':
                        # Spawn a slow projectile aimed at the player
                        to_player = player_pos - thing_pos
                        to_len = glm.length(to_player)
                        if to_len > 0.001:
                            self.projectiles.append({
                                'pos':    glm.vec3(thing_pos),
                                'vel':    (to_player / to_len) * MONSTER_PROJECTILE_SPEED,
                                'damage': damage,
                                'dist':   0.0,
                            })
                    else:
                        # Ground monsters deal instant damage
                        self.player_health = max(0, self.player_health - damage)
                ...
                # Ground monsters still fall when out of sight range
                if mtype != 'flying':
                    state['vel_y'] += GRAVITY * delta
                    if state['vel_y'] < TERMINAL_VELOCITY:
                        state['vel_y'] = TERMINAL_VELOCITY
    ...
    def _update_projectiles(self, delta: float):
        """
        Move flying-monster projectiles and test for wall or player collision.
        Projectiles that hit a wall, reach max range, or touch the player are removed.
        """
        if not self.projectiles:
            return

        player_pos     = self.player.pos if self.player else None
        HIT_RADIUS_SQ  = 40.0 ** 2   # player hit sphere radius (world units)

        alive = []
        for proj in self.projectiles:
            move = proj['vel'] * delta
            proj['pos'] += move
            proj['dist'] += glm.length(move)

            # Despawn when it has travelled too far
            if proj['dist'] >= MONSTER_PROJECTILE_MAX_DIST:
                continue

            # Wall collision — point-in-brush test
            px, py, pz = proj['pos'].x, proj['pos'].y, proj['pos'].z
            hit_wall = False
            for brush in self.brushes:
                if (brush.get('hidden') or brush.get('is_water') or
                        brush.get('is_fog') or brush.get('is_trigger')):
                ...
                bp = brush.get('pos',  [0, 0, 0])
                bs = brush.get('size', [64, 64, 64])
                if (px > bp[0] - bs[0] * 0.5 and px < bp[0] + bs[0] * 0.5 and
                        py > bp[1] - bs[1] * 0.5 and py < bp[1] + bs[1] * 0.5 and
                        pz > bp[2] - bs[2] * 0.5 and pz < bp[2] + bs[2] * 0.5):
                    hit_wall = True
                    break
            if hit_wall:
                continue
            ...
            # Player hit
            if player_pos is not None and not self.player_dead:
                d = proj['pos'] - player_pos
                if d.x * d.x + d.y * d.y + d.z * d.z < HIT_RADIUS_SQ:
                    self.player_health = max(0, self.player_health - proj['damage'])
    ...
    def _prepare_render_state(self):
        ...
        # Transfer active projectiles for rendering
